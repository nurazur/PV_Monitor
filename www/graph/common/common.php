...
// Version 07.01.2017 : use ISO year from php function
...
//          28.12.   29.12.  30.12.  31.12.  01.01.  02.01.  03.01.  04.01. 05.01
//php       52       01      01      01      01      01      01      01     02
//python    51       52      52      52      00      00      00      00     01
...
function calc_python_week_from_time_stamp($day_time)
{
    ...
    $day = date("W d.m.Y", $day_time);
    errorlog("../debug_sunezy.txt", "Tag: ".$day);
    errorlog("../debug_sunezy.txt", "Python Woche: ".$python_week);
    $c_month = intval(date("m", $day_time));
    errorlog("../debug_sunezy.txt", "Monat: ".$c_month);
...
function calc_logfilename($timestamp, $sensor = "bmp180")
{
    ...
    {
        errorlog("/var/www/graph/debug_sunezy.txt", "logfile name is empty because it does not exist: ".$logfile."\n");
        $logfile ="";
    }
    else
    {
        // errorlog("/var/www/graph/debug_sunezy.txt", "logfile name: ".$logfile);
    }    
...
{
    // errorlog("/var/www/graph/debug_sunezy.txt", "start time: ".$start_time." stop time: ".$stop_time);
    $list = array();
    
    $day_of_week = intval(date("N", $start_time));
    // errorlog("/var/www/graph/debug_sunezy.txt", "Day of Week: ".$day_of_week);
    $next_time = $start_time - ($day_of_week-1) * 86400; // Timestamp of start of week
    ...
    while ($next_time < $stop_time)
    {
        ...
        {
            // errorlog("/var/www/graph/debug_sunezy.txt", "break");
            break;
        }
